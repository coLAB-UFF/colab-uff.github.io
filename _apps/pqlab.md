---
title: "pqLAB"
excerpt: >
  Aplicativo de gestão de rotinas de pesquisa do coLAB/UFF, criado para
  apoiar o acompanhamento de tarefas e processos do dia a dia do laboratório.
order: 2
authors: "Viktor Chagas"
year: 2026
demo_url: "https://pqlab.colab.meme"
install_url: "https://github.com/ombudsmanviktor/pqlab"
doi: "10.5281/zenodo.22756527"
image: "/assets/img/apps/pqlab-featured.png"
---

### Créditos

Autoria: **Viktor Chagas**{% include orcid.html url="https://orcid.org/0000-0002-1806-6062" %}

Realização: coLAB/UFF

### Como citar este aplicativo

```
CHAGAS, V. pqLAB (Software). Niterói: coLAB/UFF, 2026. {% if page.doi and page.doi != "10.0000/PENDENTE" %}DOI: https://doi.org/{{ page.doi }}.{% elsif page.demo_url %}Disponível em: {{ page.demo_url }}.{% elsif page.access_url %}Disponível em: {{ page.access_url }}.{% endif %}
```
